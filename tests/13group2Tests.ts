...
describe("Group test 2", () => {
	let group: Group, group1: Group, group2: Group, child_group: Group, connected_group: Group, child_group_connected_group: Group;

	let user0: User, user1: User, user2: User;
	...
	it("should create a connected group", async function() {
		...
		assert.equal(connected_group.data.group_id, id);
		assert.equal(connected_group.data.access_by_group_as_member, group.data.group_id);
	});
	...
	it("should do a key rotation in the connected group", async function() {
		...
		assert.notEqual(old_late_key, new_late_key);
	...
	it("should access the connected group over the user class", async function() {
		...
		assert.equal(group_c.data.newest_key_id, connected_group.data.newest_key_id);
		assert.equal(group_c.data.access_by_group_as_member, connected_group.data.access_by_group_as_member);
	});
	...
	it("should create a child group from the connected group", async function() {
		...
		assert.equal(child_group_connected_group.data.group_id, id);
		assert.equal(child_group_connected_group.data.access_by_group_as_member, group.data.group_id);
	});
	...
	it("should access the child group of the connected group without loading the other groups before", async function() {
		...
		assert.equal(group_c_c.data.access_by_group_as_member, group.data.group_id);
	});
	...
	it("should access the group after invite", async function() {
		...
		assert.equal(group_c.data.access_by_group_as_member, group2.data.group_id);
	});
	...
	it("should send join req from group 2 to the new group", async function() {
		...
		assert.equal(joins[0].group_id, connected_group.data.group_id);
	});
	...
	it("should get the join req in the join req list", async function() {
		...
		assert.equal(joins.length, 1);
		assert.equal(joins[0].user_id, group1.data.group_id);
		assert.equal(joins[0].user_type, 2);
	});
	...
	it("should access the group after accepting group join req", async function() {
		...
		assert.equal(group_c.data.access_by_group_as_member, group1.data.group_id);
	});
